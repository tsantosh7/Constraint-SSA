function [com, epc_cost] = fusion_gauss_copula_spoofTQ(expe, chosen, copula_, com, draw)
...
for d=1:2,
  for k=1:12,
    ...
    priors =ones(1,12);
    priors = priors / sum(priors([1:11]));
    sum_= priors(1) * llh{1} + priors(2) * llh{2} + ...
      priors(3) * llh{3} + priors(4) * llh{4} + ...
      priors(5) * llh{5} + priors(6) * llh{6} + ...
      priors(8) * llh{8} + ...
      priors(9) * llh{9} + priors(10) * llh{10} + ...
      priors(11) * llh{11} + priors(12) * llh{12};
...
if ~isempty(find(draw==3, 1)),
  ...
  plot(expe.dset{d,8}(:,1),expe.dset{d,8}(:,2),'r.');
  plot(expe.dset{d,9}(:,1),expe.dset{d,9}(:,2),'b*');
  plot(expe.dset{d,10}(:,1),expe.dset{d,10}(:,2),'gd');
  plot(expe.dset{d,11}(:,1),expe.dset{d,11}(:,2),'m^');
  plot(expe.dset{d,12}(:,1),expe.dset{d,12}(:,2),'yv');
