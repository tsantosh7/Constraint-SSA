function draw_theory(chosen, param, signs, lw)
...
hC = my_plot_ellipses(mu_C', cov_C, 1, signs{3}, lw(1));
hI = my_plot_ellipses(mu_I', cov_I, 1, signs{4}, lw(2));

plot(mu_C(1), mu_C(2), signs{1}, 'MarkerSize', 20, 'LineWidth', 3);
plot(mu_I(1), mu_I(2), signs{2}, 'MarkerSize', 20, 'LineWidth', 3);
grid on;
...
function h = my_plot_ellipses(mu, sigma, weight, signs,lw);
...
for c = 1:C
	...
	h(c) = plot3(x,y,z, signs,'linewidth',lw);
end;
drawnow;
%hold off