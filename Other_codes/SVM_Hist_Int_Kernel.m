function [pred_labels, scores] = SVM_Hist_Int_Kernel(X,Y,X_test,C,bins)

zscoreX  = zscore(X);
for i = 1:1:size(X,1)
     hist_zscoreX(i,:) = hist(X(i,:),bins);       
end

zscoreX_test  = zscore(X_test);
for i = 1:1:size(X_test,1)
     hist_zscoreX_test(i,:) = hist(X_test(i,:),bins);       
end
